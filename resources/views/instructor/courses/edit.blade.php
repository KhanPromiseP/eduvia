...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('instructor.courses.update', $course) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $course->price) }}" step="0.01" min="0"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500
                            {{ !$course->is_premium ? 'disabled:bg-gray-100 disabled:text-gray-500' : '' }}"
                    {{ !$course->is_premium ? 'disabled' : '' }}>
                @error('price')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="number" name="duration" id="duration" value="{{ old('duration', $course->duration) }}" min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('duration')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
         ...
         <div class="mb-6">
            ...
            <textarea name="objectives" id="objectives" rows="3"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      placeholder="What will students learn from this course?">{{ old('objectives', $course->objectives) }}</textarea>
        ...
        <div class="mb-6">
            ...
            <textarea name="target_audience" id="target_audience" rows="2"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      placeholder="Who is this course for?">{{ old('target_audience', $course->target_audience) }}</textarea>
            <p class="mt-1 text-xs text-gray-500">Minimum 50 characters</p>
        ...
        <div class="mb-6">
            ...
            <textarea name="requirements" id="requirements" rows="2"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      placeholder="What do students need to know before taking this course?">{{ old('requirements', $course->requirements) }}</textarea>
            <p class="mt-1 text-xs text-gray-500">Minimum 50 characters</p>
            @error('requirements')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
