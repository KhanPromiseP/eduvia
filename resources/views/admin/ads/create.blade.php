            ...
            <form action="{{ route('admin.ads.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <div id="content-help" class="text-sm text-gray-500 mb-2">
                        Enter the appropriate content based on the ad type
                    </div>
                    ...
                    {{-- Preview --}}
                    <div id="content-preview" class="mt-4 border p-2 rounded bg-gray-50 hidden"></div>
                </div>
                ...
                <div class="border-t pt-6">
                    <button type="button" id="toggle-advanced" class="text-sm text-blue-600 mb-4">Show Advanced Options</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex items-center">
                        ...
                        <label for="is_random" class="ml-2 block text-sm text-gray-900">Random Display</label>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-6 border-t">
                    <a href="{{ route('admin.ads.index') }}" 
                       class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">Cancel</a>
                    <button type="submit" 
                            class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Create Ad</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // JSON validation for targeting
    const targetingField = document.getElementById('targeting');
    targetingField.addEventListener('blur', function() {
        if (this.value.trim()) {
            try { JSON.parse(this.value); this.classList.remove('border-red-300'); this.classList.add('border-green-300'); }
            catch(e){ this.classList.remove('border-green-300'); this.classList.add('border-red-300'); }
        }
    });

    // Dynamic content help & recommended sizes
    ...
    const helpTexts = {
        'image': 'Enter the image URL (recommended size 1200x300px for header banners).',
        'video': 'Enter the video URL (e.g., mp4).',
        'banner': 'Enter HTML code for banner ads.',
        'popup': 'Enter HTML content for pop-up ads.',
        'persistent': 'Enter HTML content for persistent bottom banners.',
    ...
    typeField.addEventListener('change', function() {
        ...
        contentHelp.textContent = helpTexts[type] || 'Enter the appropriate content for your ad type';

        // Filter placement options
        Array.from(placementField.options).forEach(option => {
            option.style.display = placementAllowed[type]?.includes(option.value) ? 'block' : 'none';
        });
        ...
        if(type==='image'){ 
            contentTextarea.addEventListener('input', function(){
                previewDiv.innerHTML = `<img src="${this.value}" class="max-w-full max-h-48"/>`;
                previewDiv.style.display = this.value ? 'block' : 'none';
            });
